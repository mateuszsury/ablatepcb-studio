...
LightBurn listens for documented automation commands on `127.0.0.1:19840` and returns responses on port `19841`. This app uses `PING`, `STATUS`, `IMPORT:<path>`, and `START`.
...
Generated PNG masks are imported into the current LightBurn project. The application then selects the newly centered image, applies its physical size and configured placement, and reads the X/Y/width/height fields back. A successful message is shown only when the verified image lower-left coordinate matches the project settings.
